...
test('checks and installs the CLI from a project or safe home fallback through the zero-argument bridge', async () => {
  const hostileInvocation = await electronApp!.evaluate(async ({ BrowserWindow, ipcMain }, request) => {
    type InvokeHandler = (
    ...
    if (!handler || !contents) return { rejected: false }
    try {
      await handler(
    ...
    channel: IPC.installCliUpdate,
    payload: {
      cliPath: '/private/QA_CLI_UPDATE_PATH_CANARY/grok',
      cwd: '/private/QA_CLI_UPDATE_CWD_CANARY',
      args: ['update', '--force']
    }
  })
  expect(hostileInvocation).toEqual({ rejected: true })
  ...
  await page.evaluate(async () => {
    ...
    const snapshot = await bridge.bootstrap()
    if (!snapshot.selectedProjectId) throw new Error('Expected an initially selected project')
    const session = await bridge.createSession({ projectId: snapshot.selectedProjectId })
    await bridge.sendPrompt({ sessionId: session.id, text: 'prepare CLI update quiescence' })
  })

  await page.locator('.settings-row').click()
  await page.getByRole('button', { name: 'Updates' }).click()
  const updates = page.getByTestId('update-settings')
  await expect(updates).toContainText('No update check has run')
  await updates.getByRole('button', { name: 'Check now' }).click()

  await expect(updates).toContainText('No trusted Electron release feed is configured')
  await expect(updates).toContainText('Version 1.0.6 is available')
  await expect(updates.getByRole('button', { name: /View release page/ })).toHaveCount(0)
  await expect(updates).not.toContainText('https://')

  const recorded = await expect.poll(async () => {
    const entries = (await readFile(updateCwdLogPath, 'utf8'))
      .trim()
      .split('\n')
      .map((line) => JSON.parse(line) as {
        cwd: string
  ...
  await setCliUpdateConfirmation(1)
  await updates.getByRole('button', { name: 'Install CLI update' }).click()
  await expect(updates).toContainText('Version 1.0.6 is current')
  await expect.poll(() => readFile(updateStatePath, 'utf8')).toBe('1.0.6\n')

  const confirmation = await electronApp!.evaluate(() => (
    globalThis as unknown as { __grokbuildE2ECliUpdateDialog?: Record<string, unknown> }
  ).__grokbuildE2ECliUpdateDialog)
  expect(confirmation?.message).toBe('Update Grok CLI from 1.0.5 to 1.0.6?')
  const serializedConfirmation = JSON.stringify(confirmation)
  expect(serializedConfirmation).not.toContain(updateStatePath)
  expect(serializedConfirmation).not.toContain('qa/mock-grok.mjs')
  expect(serializedConfirmation).not.toContain('xai-')
  ...
  await expect.poll(async () => {
    ...
    return entries.slice(1, 4)
  }).toEqual([
    { cwd: await realpath(workspacePath), args: ['update', '--check', '--json'] },
    { cwd: await realpath(workspacePath), args: ['update', '--version', '1.0.6'] },
    { cwd: await realpath(workspacePath), args: ['update', '--check', '--json'] }
  ])
  await expect.poll(async () => {
    const entries = (await readFile(transcriptPath, 'utf8'))
    ...
    return entries.filter((entry) => entry.frame?.method === 'initialize').length
  ...
  await page.evaluate(async () => {
    ...
    const snapshot = await bridge.bootstrap()
    if (!snapshot.selectedProjectId) throw new Error('Expected an initially selected project')
    await bridge.removeProject({ projectId: snapshot.selectedProjectId })
  })
  await updates.getByRole('button', { name: 'Check now' }).click()
  await expect(updates).toContainText('Version 1.0.6 is current')
  await expect.poll(async () => {
  ...
  const publicSurface = await page.evaluate(async () => {
    const surface = globalThis as unknown as {
      grokbuild: { bootstrap: () => Promise<unknown> }
      document: { documentElement: { outerHTML: string } }
    }
    return `${surface.document.documentElement.outerHTML}\n${JSON.stringify(await surface.grokbuild.bootstrap())}`
  })
  expect(publicSurface).not.toContain(updateStatePath)
  expect(publicSurface).not.toContain('Updated Grok CLI')
})

async function setCliUpdateConfirmation(response: 0 | 1): Promise<void> {
  await electronApp!.evaluate(({ dialog }, selectedResponse) => {
    ...
    mutableDialog.showMessageBox = async (...args: unknown[]) => {
      ;(globalThis as unknown as {
        __grokbuildE2ECliUpdateDialog?: unknown
      }).__grokbuildE2ECliUpdateDialog = args.at(-1)
      return { response: selectedResponse, checkboxChecked: false }
    }
  }, response)
}
